...
class MyOwlApp extends Component {
    static template = 'learn_qweb_templates.MyOwlApp'
    ...
    setup() {
        ...
        onWillStart(async ()=> {
            const data = await this.orm.searchRead("res.partner", [], ['name'], { limit: 10, order: "id desc" });
    ...
    async newPartner(){
        ...
        this.state.partners = await this.orm.searchRead("res.partner", [], ['name'], { limit: 10, order: "id desc" });
        this.state.name = ""
        this.env.services.notification.add("New Partner Added", {type: "success"})
    }
}
